...
class GenChapterIn(BaseModel):
    ...
    prompt: Optional[str] = Field("", description="Brief per guidare la generazione del capitolo", example="Racconta l’incipit della storia in tono avventuroso.")
    outline: Optional[str] = Field("", description="Outline opzionale del capitolo", example="1) Scena iniziale\n2) Piccolo conflitto\n3) Hook finale")
    book_id: Optional[str] = Field(None, description="ID del libro a cui associare il capitolo (se esiste)", example="bk_987654")
    abstract: Optional[str] = Field(None, description="Riassunto breve del capitolo (facoltativo)", example="Il protagonista sente la chiamata all’avventura.")
...
class GenChapterOut(BaseModel):
    chapter_id: Optional[str] = Field(None, description="ID del capitolo generato", example="ch_12345678")
    title: str = Field(..., description="Titolo del capitolo", example="Capitolo 1: L’inizio")
    content: str = Field(..., description="Contenuto testuale generato del capitolo", example="C’era una volta...")
    abstract: Optional[str] = Field(None, description="Riassunto breve del capitolo", example="Il protagonista sente la chiamata all’avventura.")
    page_numbers: bool = Field(..., description="Indica se il PDF generato include la numerazione delle pagine", example=True)
