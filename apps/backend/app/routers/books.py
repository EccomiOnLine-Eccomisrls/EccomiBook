...
def _book_index() -> List[Dict[str, Any]]:
    """
    Restituisce l'indice libri in memoria; se non presente, lo ricarica da disco.
    """
...
def _save_index(books: List[Dict[str, Any]]) -> None:
    """
    Salva su disco e aggiorna la cache process.
    """
...
def _ensure_book_folder(book_id: str) -> Path:
    """
    Crea la cartella dei capitoli del libro se non esiste.
    """
...
def create_book(data: BookIn):
    # genera ID (slug + orario + 4 cifre random per evitare collisioni)
    ...
    book = {
        ...
        "created_at": _now_iso(),
        "updated_at": _now_iso(),
        "chapters": [],  # parte vuoto
    }

    # crea cartella e salva indice
...
def delete_book(book_id: str):
    ...
    # elimina cartella capitoli e file eventuali
    folder = storage.CHAPTERS_DIR / book_id
    if folder.exists():
...
def books_refresh():
    """
    Scansiona storage/chapters/* e ricostruisce l'indice libri in base ai file reali.
    """
    ...
    if root.exists():
        for book_dir in sorted([p for p in root.iterdir() if p.is_dir()]):
            ...
            # .txt se manca .md per quello stesso id
