 ...
 * src/main.js — v2 (nav capitoli, autosave, export)
 * ========================================================= */
...
function showEditor(bookId) {
  ...
  // toolbar a sinistra se presente
  const tb = $("#editor-toolbar"); if (tb) tb.style.justifyContent = "flex-start";

  // default nuovo capitolo
  const chInput = $("#chapterIdInput"); if (!chInput.value) chInput.value = "ch_0001";
...
async function refreshChaptersList(bookId) {
  ...
  try {
    ...
    uiState.chapters = chapters.map(c => ({
      id: c?.id||"", title: c?.title||c?.id||"", updated_at: c?.updated_at||"", path: c?.path||""
    }));
...
function renderChaptersList(bookId, chapters) {
  ...
  chapters.forEach((ch) => {
    ...
    li.className = "card"; li.style.margin = "8px 0";
    li.innerHTML = `
      <div style="display:flex;align-items:center;justify-content:space-between;gap:8px">
        ...
        <div class="row-right" style="justify-content:flex-start;gap:6px">
...
function editChapter(cid){ $("#chapterIdInput").value = cid; uiState.currentChapterId = cid; openChapter(uiState.currentBookId, cid); }
...
async function exportBook(bookId){
  const fmt = askFormat("pdf"); if (!fmt) return;
  if (fmt === "pdf") {
    // opzionale: se/quando hai endpoint di export libro
    try {
      ...
      return url ? window.open(url,"_blank") : toast("Export avviato ma nessun URL ricevuto.");
    ...
    try {
      ...
      a.download = `${bookId}.${fmt}`; a.click(); URL.revokeObjectURL(a.href);
...
async function createBookSimple() {
  ...
  try {
    ...
    toast("✅ Libro creato!"); await toggleLibrary(true); await fetchBooks(); setTimeout(fetchBooks, 300);
...
async function renameBook(bookId, oldTitle){
  ...
  toast("✏️ Titolo modificato (endpoint reale di rename arriverà a breve).");
}
